...
export async function registerRoutes(
  ...
  app.post("/api/diary/ai-feedback", async (req, res) => {
    try {
      ...
      // Map recentEntries to include necessary fields
      const mappedRecentEntries = (recentEntries || []).slice(0, 10).map((e: any) => ({
      ...
      // Use buildDiaryPrompt with isFollowUp = false
      const prompt = buildDiaryPrompt(
        {
          entryType,
          painLevel,
  ...
  app.post("/api/diary/follow-up", async (req, res) => {
    try {
      ...
      const { originalEntry, followUpQuestion, assessment, recentEntries } = req.body;
      ...
      // Use buildDiaryPrompt with isFollowUp = true
      const prompt = buildDiaryPrompt(
        {
          entryType: originalEntry.entryType,
          painLevel: originalEntry.painLevel,
          sentiment: originalEntry.sentiment,
          entryText: followUpQuestion, // The follow-up question is the new "entry text"
        },
        {
          formData: assessment,
          analysis: assessment.analysis,
        ...
        mappedRecentEntries,
        true // isFollowUp = true for follow-ups
      );
  ...
  app.post("/api/diary/insights", async (req, res) => {
    try {
      ...
      const { entries, assessment } = req.body;
      ...
      // Build prompt using the modular template
